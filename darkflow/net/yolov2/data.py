from ...utils.pascal_voc_clean_xml import pascal_voc_clean_xml, pascal_voc_clean_xml_no_printing
...
def parse_valid(self, exclusive=False):
    meta = self.meta
    ext = '.parsed'
    ann = self.FLAGS.annotation_valid
    if not os.path.isdir(ann):
        msg = 'Annotation directory not found {} .'
        exit('Error: {}'.format(msg.format(ann)))
    dumps = pascal_voc_clean_xml_no_printing(ann, meta['labels'], exclusive)
    return dumps

def batch_per_epoch(self, exclusive=False):
    meta = self.meta
    ext = '.parsed'
    ...
    if not os.path.isdir(ann):
        ...
        exit('Error: {}'.format(msg.format(ann)))
    dumps = pascal_voc_clean_xml_no_printing(ann, meta['labels'], exclusive)
    return int(len(dumps) / self.FLAGS.batch)
...
def _validation(self, chunk):
    ...
    w, h, allobj_ = chunk[1]
    allobj = deepcopy(allobj_)
    path = os.path.join(self.FLAGS.validation, jpg)
    img = self.preprocess(path, allobj)
    ...
    for i in range(1):
        ...
        for b in range(batch_per_epoch):
            ...
            for j in range(b * batch, b * batch + batch):
                ...
                try:
                    inp, new_feed = self._validation(train_instance)
