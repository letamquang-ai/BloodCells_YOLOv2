...
for file_name in os.listdir(directory):
    ...
    for prediction in output:
        ...
        if label == 'RBC' and confidence < .5:
            continue
        if label == 'WBC' and confidence < .3:
            continue
        if label == 'Platelets' and confidence < .5:
            continue
        ...
        image = cv2.putText(image, f'{confidence:.2f}', (center_x - 15, center_y + 20), font, .5, color, 1)
